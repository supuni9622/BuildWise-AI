...
class AssumptionDraft(BuildWiseModel):
    ...
    Intentionally has no ``requires_validation``/``validation_question``
    consistency check: ``Assumption`` (the canonical model this assembles
    into) already enforces it, and enforcing it here too only risks a raw
    parse-time crash that bypasses the guardrail retry loop — the exact
    failure shape ``domain.artifact_drafts._draft_model`` strips for every
    other artifact. This file predates that shared mechanism, so the same
    fix is applied by hand here instead.
    """
...
class UnknownDraft(BuildWiseModel):
    ...
    Intentionally has no ``can_proceed_with_assumption``/
    ``recommended_assumption`` consistency check — see ``AssumptionDraft``.
    ``Unknown`` (canonical) already enforces it.
    """
...
class ClarificationQuestionDraft(BuildWiseModel):
    ...
    Intentionally has no choice/options consistency check — see
    ``AssumptionDraft``. ``ClarificationQuestion`` (canonical) already
    enforces it.
    """
...
class DiscoveryDraft(BuildWiseModel):
    ...
    Intentionally has no cross-field semantic-consistency check (unique
    unknown keys, fact/assumption key overlap, clarification questions
    referencing real unknown keys) — see ``AssumptionDraft``.
    ``DiscoveryResult.validate_discovery_result`` (canonical) already
    enforces the reference-integrity checks, by ID rather than by key, once
    ``assemble_discovery_result`` builds it. Reproduced live: an LLM output
    that failed exactly this check crashed here at raw parse time, with zero
    guardrail retries, identical in shape to the original crash this file
...
def _assemble_questions(
    ...
    *,
    ...
    for item in draft.questions:
        missing = [key for key in item.related_unknown_keys if key not in unknown_by_key]
        if missing:
            raise ValueError(
                f"Clarification question '{item.key}' references unknown keys "
                f"that are not present in unknowns: {', '.join(sorted(missing))}."
            )
